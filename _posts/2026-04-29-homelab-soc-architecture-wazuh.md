...
During setup, VMware's default "Automatic" bridging caused IP assignment failures. I remediated this by hard-coding the VMnet0 virtual switch to the physical host network adapter and implementing a strict YAML Netplan configuration within Ubuntu to enforce a static IP and disable DHCP drift.
...
- **Local SAM Account:** Utilized `OOBE\BYPASSNRO` to bypass Microsoft Account requirements, ensuring consistent forensic testing with a local account.
...
## Phase 4: Telemetry Enrichment (Sysmon Integration)
...
## Phase 5: Adversarial Simulation Readiness
...
- **Framework:** Deployed the **Invoke-AtomicRedTeam** execution framework.
...
This lab build provides a robust foundation for detection engineering and security analysis. By combining Wazuh's SIEM/XDR capabilities with high-fidelity telemetry from Sysmon, I have created a realistic "playground" for analyzing modern cyber threats.
